...
benchmark_blasso_2BG_beta_sigma2 <- function(
    vy,
    mX,
    lambda_init,
    sigma2_init,
    ...
    beta_inds=NA)
{
  ...
  for (i in seq_len(trials)) {
    time_val = system.time({
      res_mcmc = blasso_gibbs_2block_bs(vy, mX, a, b, u, v, nsamples, lambda_init, sigma2_init,   nsamples/5)
    })[3]
    # print(time_val)
...
benchmark_blasso_2BG_beta_lambda2 <- function(
    vy,
    mX,
    lambda_init,
    sigma2_init,
    ...
    beta_inds=NA)
{
  ...
  for (i in seq_len(trials)) {
    time_val = system.time({
      res_mcmc = blasso_gibbs_2block_bl(vy,
                                          mX,
                                          a,
                                          b,
                                          u,
